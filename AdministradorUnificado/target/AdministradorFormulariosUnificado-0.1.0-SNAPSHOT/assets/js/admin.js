$(document).on('change', '#formulario', function(event) {
$("#nombreAdmin").text("Administrador " + $("#formulario option:selected").text());
});
...
function generarTabla(jsonStr){
	...
	txt += "</tbody>";
	document.getElementById("data-table").innerHTML = txt;
	numRowsInfo(numRows);
...
function exportarExcelXLSX(fechaInicio, fechaFin){
	...
	if(validaFecha(fechaInicio, fechaFin) === -1){
		...
		request.open('POST', "/administrador");
...
$("#btn_fecha").click(function(){
	if($('#fechaInicio').val() != "" && $('#fechaFin').val() != ""){
		listarPorFechas($('#fechaInicio').val(), $('#fechaFin').val());
	}else{
		$(".label__error").show();
		$('#fechaInicio').addClass("error_v");
		$('#fechaFin').addClass("error_v");
		$('.red').addClass("error_v");
		$('.red_1').addClass("error_v");
	}
});

$('#fechaInicio').click(function(){
	$(".label__error").hide();
	$('#fechaInicio').removeClass("error_v");
	$('.red').removeClass("error_v");
});

$('#fechaFin').click(function(){
	$(".label__error").hide();
	$('#fechaFin').removeClass("error_v");
	$('.red_1').removeClass("error_v");
});
